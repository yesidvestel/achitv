...
<div class="invoice-box" >
                ...
                if(count($transacciones)!=0){                    
                    ...
                    if($pa=="no"){
                            echo '<tr class="item' . $flag . '"> 
                                <td><strong>' . strftime("%B", strtotime($f1)). ' CTA:'. $invoice['tid'].'</strong></td>';
                            echo '<td class="t_center"><strong>' . amountExchange($valor) . '</strong></td>
                                        </tr>';    
                    }
                    ...
                    foreach ($lista_items as $key => $value) {
                        ...
                        echo '<tr class="item' . $flag . '"> 
                                <td><strong>'.$value->product.'</strong></td>';
                        echo '<td class="t_center"><strong>' . amountExchange( $valor_item) . '</strong></td>
                                </tr>';
 ...
 //codigo resien agregado editar con caso en el que el primer item sea el pagado adelantado
            ...
            echo '<tr class="item' . $flag . '"> <td><strong>' . strftime("%B", strtotime($f1)). ' CTA:'. $factura['tid'].'</strong></td>';
            echo '<td class="t_center"><strong>' . amountExchange( $valor) .'</strong></td></tr>';
            $cantidad_total_a_restar-=$cantidad_total;
            }
...
if($vrm>0){
    ...
    foreach ($facturas_adelantadas_list as $key => $value) {
                    echo '<tr class="item' . $flag . '"> 
                                    <td><strong>' . ucfirst($value['mes']). '</strong></td>';
                        echo '<td class="t_center"><strong>' . amountFormat($value['valor_a_colocar']) . '</strong></td>
                                    </tr>';
    }
  ...
  if ($invoice['shipping'] > 0) { $cols++;}
    ...
    <table class="subtotal">
        ...
        <tr class="f_summary">
            ...
            <td><strong><?=($pa=="si") ? '$ 0' : amountExchange(($cantidad_total)+$cantidad_total_a_restar); ?></strong></td>
        </tr>
        <?php 
        if ($invoice['discount'] > 0) {
            ...
            <td><strong>' . $this->lang->line('Total Discount') . ':</strong></td>

            <td><strong>' . amountExchange($invoice['discount2'], $invoice['multi2']) . '</strong></td>
        </tr>';
        ...
        ?>
        <tr>
			<td><strong><?php echo $this->lang->line('Paid Amount')?></strong></td>

            <td><strong><?php echo amountExchange($cantidad_total_a_restar); ?></strong></td>
		</tr><tr>
            <td><strong><?php echo $this->lang->line('Balance Due') ?>:</strong></td>
    ...
    echo '</strong></td>
		...
		<div class="sign2"><strong>(' . $user . ')</strong></div><div class="sign3"><strong>' . user_role($perfil) . '</strong></div> <div class="terms"><hr><strong>' . $this->lang->line('Terms') . ':</strong>';

    echo '<strong>' . $invoice['termtit'] . '</strong><br><strong>' . $invoice['terms']."</strong>";
    ?></div>
</div>
</body>
</html>