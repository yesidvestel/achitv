...
//var_dump($data['estado_villanueva_epon']);
    ...
    public function explortar_a_excel(){
        ...
        if (isset($_GET['direccion']) &&$_GET['direccion'] =="Personalizada"){
          ...
          if($_GET['sel_tipo_documento']!=""){
                        if($condicionales!=""){
                            $condicionales.=" and " ;   
                        }
                        $condicionales.="  tipo_documento = '".$_GET['sel_tipo_documento']."' ";            
        }
        if($_GET['busqueda']!=""){
                        ...
                        if($condicionales!=""){
                         ...
                         $column_search = array('abonado','name', 'celular', 'documento', 'unoapellido', 'email','usu_estado');
                         //$condicionales.="  (documento like '%".$this->input->post('search')['value']."%' )";   
                         $condicionales.="  (";   
                    foreach ($column_search as $kx1 => $vx1) {
                     $condicionales.="  ".$vx1." like '%".$_GET['busqueda']."%' ";   
                         if($kx1<(count($column_search)-1)){
                            $condicionales.=" or ";
                         }
                    }
                        $condicionales.=")";
                    ...
                    if($fact_valida){
                        if($_var_tiene_tv){
                            ...
                            //}else{
                                ...
                                $producto=$this->db->get_where('products', array("pid"=>"3"))->row();
                            ...
                            if($producto!=null){
                                ...
                                $producto=$this->db->get_where('products', array("pid"=>"20"))->row();
                                
                                if($producto!=null){
                                    $suma+=$producto->product_price;
                                    $suscripcion_str="Tv";
                                }else {
                                        $producto=$this->db->get_where('products', array("pid"=>"21"))->row();    
                                        if($producto!=null){
                                            $suma+=$producto->product_price;
                                            $suscripcion_str="Tv";
                                        }
...
//esto es para los estados
    ...
    $headers = array('Abonado' => 'string','Cedula' => 'string', 'Nombre' => 'string', 'Celular' => 'string', 'Direccion' => 'string',"Referencia"=>"string",'Barrio' => 'string','Serv. Suscritos' => 'string', 'Tegnologia' => 'string','Estado' => 'string','Deuda' => 'integer','Suscripcion' => 'integer','Ingreso' => 'integer');
    if($_GET['ultimo_estado_sel']=="Si"){
    ...
    foreach ($lista_customers2 as $key => $customer) {            
            $direccion= $customer->nomenclatura . ' ' . $customer->numero1 . $customer->adicionauno.' Nº '.$customer->numero2.$customer->adicional2.' - '.$customer->numero3;
                    ...
                    $str_barrio="";
                            ...
                            if($_GET['ultimo_estado_sel']=="Si"){
                                $writer->writeSheetRow('Customers '.$cust_group->title,array($customer->abonado,$customer->documento ,$customer->name.' '.$customer->dosnombre.' '.$customer->unoapellido.' '.$customer->dosapellido, $customer->celular, $direccion,$customer->referencia,$str_barrio ,$customer->suscripcion_str,$customer->tegnologia,$customer->usu_estado,$customer->deuda,$customer->suscripcion,$customer->money,$customer->ultimo_estado,$customer->fecha_ultimo_estado));                    
                            }else{
                                $writer->writeSheetRow('Customers '.$cust_group->title,array($customer->abonado,$customer->documento ,$customer->name.' '.$customer->dosnombre.' '.$customer->unoapellido.' '.$customer->dosapellido, $customer->celular, $direccion,$customer->referencia,$str_barrio ,$customer->suscripcion_str,$customer->tegnologia,$customer->usu_estado,$customer->deuda,$customer->suscripcion,$customer->money));                    
                            }
    ...
    public function load_morosos(){ 
        ...
        if($this->input->post('search')['value']!=""){
                        ...
                        if($condicionales!=""){
                         ...
                         $condicionales.="  (";   
                    foreach ($column_search as $kx1 => $vx1) {
                     $condicionales.="  ".$vx1." like '%".$this->input->post('search')['value']."%' ";   
                         if($kx1<(count($column_search)-1)){
                            $condicionales.=" or ";
                         }
                    }
...
//esto es para los estados 
	    ...
	    $datax['user_id']=$this->aauth->get_user()->id;
...
if ($valido) {
    ...
    public function cortar_usuarios_multiple(){
                        ...
                        if($tipo_corte=="Corte Combo") {
                            ...
                            $producto2 = $this->db->get_where('products',array('product_name'=>'Reconexión Television'))->row();
                                ...
                                //SELECT * FROM `invoice_items` where product like "%Reconexión Internet%"
                                    ...
                                    //actualizar factura
                                $facturax = $this->db->get_where('invoices',array('tid'=>$factura['tid']))->row();
                                    $this->db->set('subtotal', $facturax->subtotal+$producto2->product_price);
                                    $this->db->set('ron', 'Cortado');               
                                    $this->db->set('total', $facturax->total+$producto2->product_price);
                                    $this->db->set('items', $facturax->items+1);
      ...
      public function descargar_pdf_falctura_usuarios_media_carta(){
        ...
        ini_set('memory_limit', '150000000M');
