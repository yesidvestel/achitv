...
public function clean_invoices(){// 2022-08-29
    $lista=$this->db->get_where("invoices",array("invoicedate"=>"2023-01-27","notes"=>"."))->result();
...
public function sub_paquete(){
    $lista=$this->db->query("SELECT * FROM `invoices` as invo WHERE tid =(select inv.tid from invoices as inv where inv.csd=invo.csd ORDER by inv.tid desc LIMIT 1) GROUP by csd;")->result();
    foreach ($lista as $key => $inv) {
        //$inv->combo=str_replace(" ","", $inv->combo);
        //$inv->combo=strtolower($inv->combo);
        if($inv->combo=="3 Megas "){
            $inv->combo="5Megas";
            $datax=array("combo"=>$inv->combo);
            $this->db->update("invoices",$datax,array("tid"=>$inv->tid));
        }
    }
}
...
$this->load->model("Notas_model","notas");
    ...
    public function create()
    {
		...
		$data['paquetetv'] = $this->invocies->paquetestv();
    ...
    public function f12(){
        $lis=$this->db->query("select * from invoices where invoicedate='2023-09-29' and notes='.'")->result_array();
        $str="";
        foreach ($lis as $key => $value) {
            $str.="delete from invoice_items where tid=".$value['tid'].";<br>";
        }
        ob_clean();
        echo $str;
    }
        ...
        foreach ($customers_list as $key => $value) {            
            ...
            foreach ($invoices as $key => $value2) {
                ...
                //para omitir los traslados y afiliaciones
                 $afiliacion_traslado_omitir=$this->db->query('SELECT * FROM `invoice_items` where (product like "%afiliacion%" or product like "%traslado%") and tid="'.$value2->tid.'"')->result_array();
                    ...
                    if($_customer_factura_creada==false){
                            ...
                            if($_tiene_television==true && ($value2->estado_tv==null || $value2->estado_tv=='null')){
                               /* if(strpos(strtolower($caja1->holder), strtolower("mocoa"))!==false){*/
                                    $tv_product= $this->db->get_where("products", array('pid' => "3"))->row();
...
public function generar_pdf_facturas_generadas(){
    ...
    //define column headers
        ...
        //meta data info
    $keywords = array('xlsx','CUSTOMERS','ACHI');
    //$writer->setTitle('Reporte Facturas Generadas '.$sede." - ".$fecha);
    $writer->setTitle($nombrey);
    $writer->setSubject('');
    $writer->setAuthor('ACHI');
    $writer->setCompany('ACHI');
...
/**/
	...
	public function eliminarfacturas()
    {
		...
		$this->db->where('notes', '.');
		$this->db->where('invoicedate', '2023-11-30');
		...
		if (!empty($factura_ids)) {
			...
			if ($this->db->delete('invoice_items')){
				$this->db->where('invoicedate', '22023-11-30');
				$this->db->where('notes', '.');
